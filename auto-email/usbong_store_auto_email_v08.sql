...
CREATE TABLE IF NOT EXISTS `usbong_store`.`auto_email_template` (
  ...
  `data_01_type` ENUM('image', 'text', 'textarea') NULL DEFAULT 'text' COMMENT '',
  `data_02_type` ENUM('image', 'text', 'textarea') NULL DEFAULT 'text' COMMENT '',
  `data_03_type` ENUM('image', 'text', 'textarea') NULL DEFAULT 'text' COMMENT '',
  `data_04_type` ENUM('image', 'text', 'textarea') NULL DEFAULT 'text' COMMENT '',
  `data_05_type` ENUM('image', 'text', 'textarea') NULL DEFAULT 'text' COMMENT '',
...
-- -----------------------------------------------------
-- Table `usbong_store`.`auto_email_unsubscribe`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `usbong_store`.`auto_email_unsubscribe` (
  `customer_id` INT(11) NOT NULL COMMENT '',
  `datetime` DATETIME NOT NULL COMMENT '',
  INDEX `auto_email_unsubscribe_fk_customer_id` (`customer_id` ASC)  COMMENT '',
  PRIMARY KEY (`customer_id`)  COMMENT '',
  CONSTRAINT `auto_email_unsubscribe_fk_customer_id`
    FOREIGN KEY (`customer_id`)
    REFERENCES `usbong_store`.`customer` (`customer_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_unicode_ci
COMMENT = 'customer_ids placed in this table do not want to receive emails.';
